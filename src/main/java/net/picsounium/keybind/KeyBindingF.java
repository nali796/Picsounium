...
package net.picsounium.keybind;

import org.lwjgl.input.Keyboard;

import net.picsounium.procedure.ProcedureFOnKeyPressed;
...
public class KeyBindingF extends ElementsPicsounium.ModElement {
	...
	public void init(FMLInitializationEvent event) {
		keys = new KeyBinding("key.mcreator.f", Keyboard.KEY_G, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (Minecraft.getMinecraft().currentScreen == null) {
			if (org.lwjgl.input.Keyboard.isKeyDown(keys.getKeyCode())) {
				Picsounium.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
	...
	private static void pressAction(EntityPlayer entity) {
			...
			return;
		{
			...
			ProcedureFOnKeyPressed.executeProcedure($_dependencies);
		}
	}
}
